...
const postDetails: Record<string, PostDetail> = {
  "home-1": {
    ...
    content: "今天带家里猫咪出去晒太阳，真的很乖。社区里的宠物日常、救助故事和成长瞬间都放在这里。",
    city: "西安",
...
export const mockHomeState = {
  ...
  tags: HOME_TAGS,
  featuredPosts: HOME_FEEDS.map((item) =>
    toFeedCardView({
      id: item.id,
      type: "PET_SOCIAL",
      serviceCategory: null,
...
export function appendMockReply(commentId: string, content: string): CommentCreateResult {
  ...
  for (const [postId, list] of Object.entries(commentLookup)) {
    ...
    const nestedReply = list.some((item) => item.replies.some((reply) => reply.id === commentId));
    if (nestedReply) {
